...
class Calendar extends React.Component {
  ...
  componentDidMount() {
    if (typeof window !== "undefined") {window.addEventListener('unload', this.saveEvents)};
    this.setDate(this.props.day, this.props.month-1, this.state.year);
    this.loadEvents();
  }
  componentWillUnmount() {
    if (typeof window !== "undefined") {window.removeEventListener('unload', this.saveEvents)};
    this.saveEvents();
  }
  ...
  saveEvents() {
if (typeof window !== 'undefined') localStorage.setItem("events", JSON.stringify(this.state.events));
  }
  loadEvents() {
    let events;
    if (typeof window !== 'undefined') {events = localStorage.getItem("events")};
  ...
  removeEvent(date, idx) {
    ...
    let dayNames = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((item, i) => {
      return (<div key={item} className="day">{item}</div>);
    });
    ...
    let events = this.getEvents(this.state.cursor).map((item, i) => {
      return (
        <li key={i}>{item}
          <a href="#" key={i + item} onClick={() => this.removeEvent(this.state.cursor, i)}>–</a>
        </li>    
      );
    });
